...
export default function TableProxParcelas({ data }: TableProxParcelasProps) {
  ...
  const handleFilterChange = (filters: { nome: string; responsavel:String; tema: string,  statusParcelas: ParcelaStatusType | "TODOS", tipoTrabalho: TipoTrabalhoType | "TODOS"}) => {
    const { nome, responsavel, tema , statusParcelas, tipoTrabalho } = filters;
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="border-r border-gray-100">Próximo Pagamento</TableHead>
            ...
            {exibData.length > 0 ? (
                          exibData.map((trabalho) => (
                            <TableRow key={trabalho.id} className="hover:cursor-pointer" onClick={() => handleClickRow(trabalho.id)}>
                              ...
                              <TableCell className="border-r border-gray-100">{ParcelaStatus[trabalho.statusParcela as keyof typeof ParcelaStatus]}</TableCell>
                            </TableRow>
                          ))
